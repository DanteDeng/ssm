...
function initPage(domName,page){
	laypage({
        cont: domName, //容器。值支持id名、原生dom对象，jquery对象。【如该容器为】：<div id="page1"></div>
        pages: Math.ceil(page.pages), //通过后台拿到的总页数
        skin: "#429842",
        curr: page.pageNum||1, //当前页
        groups: 3, //连续显示分页数  
        skip: true, //是否开启跳页  
        first: '首页', //若不显示，设置false即可  
        last: '尾页', //若不显示，设置false即可  
        jump: function (e) { //触发分页后的回调
        	$("#"+domName+" a").attr("href",'javascript:nextQuery(\''+domName+'\')');
        	$("#"+domName+" .laypage_btn").attr("onclick",'javascript:numQuery(\''+domName+'\')');
        }
    });
}

function numQuery(domName){
	var pageNum = $("#"+domName+" .laypage_skip").val();
	load(pageNum);
}
function nextQuery(domName){
	var pageNum = $("#"+domName+" .laypage_curr").text().trim();
	load(pageNum);
}